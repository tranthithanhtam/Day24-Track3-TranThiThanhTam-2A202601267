...
def segment_vietnamese(text: str) -> str:
    ...
    try:
        ...
        # split(" ") → "nghỉ_phép" là 1 token, nhưng query "nghỉ phép" là 2 token
        # → KHÔNG khớp. Phải replace("_", " ") để BM25 hoạt động đúng.
        return segmented.replace("_", " ")
    except Exception as e:
        print(f"  ⚠️  underthesea không dùng được ({e}) — dùng text gốc.")
...
class BM25Search:
    ...
    def search(self, query: str, top_k: int = BM25_TOP_K) -> list[SearchResult]:
        ...
        return [
            ...
            if scores[i] > 0  # bỏ docs không khớp token nào
        ]
...
class DenseSearch:
    ...
    def index(self, chunks: list[dict], collection: str = COLLECTION_NAME) -> None:
        ...
        # recreate_collection() đã deprecated ở qdrant-client mới → dùng
        # delete + create để tương thích cả hai đời client.
