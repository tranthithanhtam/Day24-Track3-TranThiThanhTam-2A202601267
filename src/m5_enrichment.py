...
ENRICH_MODEL = os.getenv("ENRICH_MODEL", LLM_MODEL)
_API_DISABLED = False
...
def _json_object(content: str) -> dict:
    """Parse JSON returned by models that may wrap it in Markdown fences."""
...
def _api_failed(error: Exception) -> None:
    global _API_DISABLED
    _API_DISABLED = True
    print(f"  ⚠️  Enrichment API disabled for remaining chunks: {error}")
...
def generate_hypothesis_questions(text: str, n_questions: int = 3) -> list[str]:
    ...
    # Extractive fallback: biến từng câu thành câu hỏi.
...
def extract_metadata(text: str) -> dict:
    ...
    if OPENAI_API_KEY and not _API_DISABLED:
        try:
            ...
            return _json_object(resp.choices[0].message.content)
        except Exception as e:
...
def _enrich_single_call(text: str, source: str) -> dict:
    ...
    # Fallback không cần API: ghép kết quả extractive của 4 technique.
    return {
